...
static const GUI_WIDGET_CREATE_INFO _aFrameWinDis[] = {
  { FRAMEWIN_CreateIndirect, "11111", 		1,                10, 10, 100, 150, 0,          0 },
  { BUTTON_CreateIndirect,   "pro",    		GUI_ID_BUTTON2,      		10, 60, BUTTON_SIZE_X,  BUTTON_SIZE_Y,  0,          0 },
  { BUTTON_CreateIndirect,   "next",      GUI_ID_BUTTON3,     				46, 60, BUTTON_SIZE_X,  BUTTON_SIZE_Y,  0,          0 }//,
  /*{ PROGBAR_CreateIndirect,  0,         GUI_ID_PROGBAR0,  2, 11, PROGBAR_SIZE_X, PROGBAR_SIZE_Y, WM_CF_HIDE, 0 },
  { TEXT_CreateIndirect,     0,         GUI_ID_TEXT0,     2,  2, TEXT_SIZE_X,    TEXT_SIZE_Y,    0,          0 }*/
};
...
static WM_HWIN win_foucus_button[2][2]={{0,0},{0,0}};
...
U8  win_foucus_button_x = 0,win_foucus_button_y = 0;
...
static int _cbEffect(int TimeRem, void * pVoid) {
  ...
  Pressed = *((int *)pVoid);
  GUI_Exec();
  GUI_PID_GetState(&State);/*返回是否按下设备指针设备或touch*/
  if (State.Pressed) {
    *((int *)pVoid) = 1;
    return 0;
  } else {
