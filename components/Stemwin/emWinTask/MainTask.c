...
*	功能说明:  Draws a vertical gradient in form of a rounded bar
*	形    参：xPos0     起始位置X
*             yPos0     起始位置Y
*             xPos1     结束位置X
*             yPos1     结束位置Y
...
*********************************************************************************************************
*	函 数 名: _CreateBitmap
*	功能说明: Creates the background image in form of a rounded horizontal bar
			...
			switch (NCode) 
			{
				...
				case WM_NOTIFICATION_RELEASED:
					...
					else 
					{
						...
						if (_hStatus == 0) 
						{
							_hStatus = WM_CreateWindowAsChild(10, 20,120, 140, hWin, WM_CF_SHOW, _cbStatus, 0);
						}
					}
					break;
			}
...
static void _DrawBackground(void) 
{
	...
	GUI_SetColor(GUI_WHITE);
	GUI_FillRect(  5,  40, 100,  140);
	GUI_DrawGradientV(5, 80, 100, 140, 0xffffff, 0xffa0a0);

	GUI_UC_SetEncodeUTF8();
	GUI_SetFont(&GUI_FontYahei);
	GUI_SetTextMode(GUI_TM_TRANS);
	GUI_SetColor(GUI_WHITE);
	GUI_DispStringHCenterAt("aabb", 160, 5);
	GUI_UC_SetEncodeUTF8();
}
...
static void _DemoWashingMachine(void) 
{
	...
	hWinMain = WM_CreateWindowAsChild(5, 40, 128, 160, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, _cbMain, 0);
	/* 设置聚焦 */
	WM_SetFocus(hWinMain);
	
	/* 在窗口hWinMain上面创建按钮 */
	hButton = BUTTON_CreateEx(10, 90, 60, 60, hWinMain, WM_CF_SHOW, 0, GUI_ID_BUTTON0);
	/* 设置按钮的回调函数，本身按钮的回调函数是自动执行的，这里应该属于自定义回调函数 */
