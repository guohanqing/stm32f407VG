...
static rt_sem_t 	disp_sem = RT_NULL; 
static rt_mutex_t disp_mutex = RT_NULL; 
...
{ 
  ...
  //osMutex = osMutexCreate(osMutex(MUTEX));
	disp_mutex = rt_mutex_create("dispsem" , RT_IPC_FLAG_FIFO); 
  
  /* Create Semaphore lock */
  //osSemaphoreDef(SEM);
  /* Create the Semaphore used by the two threads */
  //osSemaphore= osSemaphoreCreate(osSemaphore(SEM), 1);  
	disp_sem = rt_sem_create("eventsem", 0,RT_IPC_FLAG_FIFO); 
}

void GUI_X_Lock(void)
{
  //osMutexWait(osMutex , osWaitForever) ;
	rt_mutex_take(disp_mutex, 0);
}
